...
def weather(message):
    ...
    if wind_speed < 5:
        ...
        bot.send_message(message.from_user.id, '❌ На улице шторм, ')
...
#         bot.send_message(message.from_user.id, 'Здравствуй. Какое тебе до меня  дело?')
#     elif message.text == '/help':
#         bot.send_message(message.from_user.id, 'Если хочешь, чтобы с тобой начали разговор, скажи "привет" для начала. ')
#     else:
#         bot.send_message(message.from_user.id, "Я тебя не понимаю, но можешь попросить помощи (/help)")
# bot.polling(none_stop=True, interval=0)
